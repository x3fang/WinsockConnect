...
#define runPluginValue &ClientSocketQueue, &ServerSocketQueue, &ServerSEIDMap, &ClientSEIDMap, &ClientMap, &ServerQueueLock, &ClientQueueLock

extern "C" int EXPORT
delForId(int ClientId)
{
...
extern "C" int EXPORT showForSend(allInfoStruct *info, string seid, filter f, bool startIf = false, ClientSocketFlagStruct::states state = ClientSocketFlagStruct::NULLs)
{
      int status = 0;
